...
The streaming regression also checks that replies after Submit, Decline, and Cancel are sent as new messages after the form, while the earlier message only receives tool status edits. A following turn without a form must resume normal streaming edits.

For a live Discord canary, use a disposable bot and channel. Trigger a form with string, number, integer, boolean, single-select, and multi-select fields. Confirm review and modification work. Confirm invalid values do not send an ACP response. Confirm users outside the active turn get an ephemeral denial for controls and a brief notice for text replies. Their answers must not enter normal agent dispatch. Trigger a long form and move to the last page; confirm no prompt, choice, default, error, or review text is missing. Trigger text fallback and complete it with `!form next`, `!form prev`, `!form choose N`, `!form review`, `!form edit N`, `!form skip` on an optional field, `!form value "hello"` on a string field, and `!form submit`, `!form decline`, and `!form cancel`. After Submit, Decline, Cancel, `/cancel`, `/reset`, and timeout, click old controls and reply to the old form message. No second ACP response must be written. Confirm **Modify** shows the `!form edit N` instructions without changing a value. Confirm `!formnext` is rejected with a spacing hint. Enter invalid JSON with `!form value` and confirm the error shows valid examples, not parser internals. Close the agent input pipe before Submit and confirm the form becomes Expired, not Submitted.
